...
static int	ft_lenght(const char *str, char c)
{
	int	i;
...
char	**ft_split(char const *s, char c)
{
	...
	if (!dest)
		return (NULL);
	while (j < ft_countwords(s, c) && *s_cpy)
	{
		while (*s_cpy == c)
			s_cpy++;
		dest[j] = malloc((ft_lenght(s_cpy, c) + 1) * sizeof(char));
		if (!dest[j])
			return (ft_my_free_split(dest));
		ft_strlcpy(dest[j], s_cpy, ft_lenght(s_cpy, c) + 1);
		s_cpy += ft_lenght(s_cpy, c);
		j++;
	}
	dest[j] = NULL;
	return (dest);
